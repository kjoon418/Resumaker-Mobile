package com.resumaker.feature.resumebuilder.screen
...
import com.resumaker.feature.resumebuilder.component.card.FeedbackOptionCard
import com.resumaker.feature.resumebuilder.component.card.GeneratedResumeContentCard
import com.resumaker.feature.resumebuilder.component.card.PlaceholderResumeCard
import com.resumaker.feature.resumebuilder.component.card.SelectionItemCard
import com.resumaker.core.ui.input.PrimaryTextField
import com.resumaker.core.designsystem.DesignSystem
import com.resumaker.core.designsystem.standardScreenPadding
...
import com.resumaker.feature.resumebuilder.preview.ResumeEditStateProvider
...
/**
 * Stateful Composable: ViewModel과만 통신.
 * UI 이벤트를 Intent로 전달하고, State를 Stateless 컴포넌트에 전달합니다.
 */
...
/**
 * Stateless Composable: 매개변수로 전달받은 데이터와 콜백 람다만 처리.
 * UI 프리뷰 및 테스트가 용이합니다.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ResumeEditContent(
    state: ResumeEditState,
    onIntent: (ResumeEditIntent) -> Unit,
    onBackClick: () -> Unit,
    onSaveClick: () -> Unit,
    onPdfExportClick: () -> Unit,
    onInterviewClick: () -> Unit,
    ...
    fun dismissBubble() = onIntent(ResumeEditIntent.DismissAiBubble)
    ...
    Scaffold(
        topBar = {
            Row(
                ...
                TextButton(onClick = { dismissBubble(); onSaveClick() }) {
                    Text("저장하기", color = colors.PrimaryBlue, fontSize = 14.sp)
                }
            }
        },
        bottomBar = {
            Box(
                ...
                Column(
                    ...
                    Row(
                        ...
                        OutlinedButton(
                            onClick = { dismissBubble(); onIntent(ResumeEditIntent.TogglePreviewMode) },
                            ...
                            Text(
                                if (state.isPreviewMode) "수정하기" else "미리보기",
                ...
                FloatingActionButton(
                    onClick = { dismissBubble(); onIntent(ResumeEditIntent.ShowAiSheet) },
        ...
        Column(
            ...
            if (state.feedbackResultText.isNotBlank()) {
                Card(
                    modifier = Modifier.fillMaxWidth(),
                    shape = RoundedCornerShape(12.dp),
                    colors = CardDefaults.cardColors(containerColor = colors.FeedbackCardBackground),
                    border = BorderStroke(1.dp, colors.PrimaryBlue.copy(alpha = 0.3f))
                ) {
                    Column(modifier = Modifier.padding(16.dp)) {
                        Row(
                            ...
                            Text(
                                text = state.feedbackResultTitle,
                                fontSize = 16.sp,
                                fontWeight = FontWeight.Bold,
                                color = colors.PrimaryBlue
                            )
                            TextButton(
                                onClick = { onIntent(ResumeEditIntent.ClearFeedbackResult) },
                                contentPadding = PaddingValues(0.dp)
                            ) {
                                Text("닫기", color = colors.TextSecondary, fontSize = 13.sp)
                            }
                        }
                        Spacer(modifier = Modifier.height(8.dp))
                        Text(
                            text = state.feedbackResultText,
                            fontSize = 14.sp,
                            color = colors.TextSecondary,
                            lineHeight = 20.sp
                        )
            ...
            if (state.isPreviewMode) {
                ...
                PrimaryTextField(
                    value = state.name,
                    onValueChange = { onIntent(ResumeEditIntent.UpdateName(it)) },
                    label = "지원 포지션 / 성함",
                ...
                OutlinedTextField(
                    value = state.career,
                    onValueChange = { onIntent(ResumeEditIntent.UpdateCareer(it)) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .heightIn(min = 120.dp)
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { onIntent(ResumeEditIntent.UpdateStrengths(it)) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .heightIn(min = 100.dp)
                        .onFocusChanged { if (it.isFocused) dismissBubble() },
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { onIntent(ResumeEditIntent.UpdateProjects(it)) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .heightIn(min = 120.dp)
                        .onFocusChanged { if (it.isFocused) dismissBubble() },
    ...
    if (state.showAiSheet) {
        ModalBottomSheet(
            onDismissRequest = { onIntent(ResumeEditIntent.HideAiSheet) },
            ...
            Column(
                ...
                FeedbackOptionCard(
                    ...
                    onClick = { onIntent(ResumeEditIntent.ShowAntiPatternSheet) }
                )
                Spacer(modifier = Modifier.height(12.dp))
                FeedbackOptionCard(
                    title = "면접관 페르소나 피드백",
                    ...
                    onClick = { onIntent(ResumeEditIntent.ShowPersonaSheet) }
                )
    ...
    if (state.showAntiPatternSheet) {
        ModalBottomSheet(
            onDismissRequest = { onIntent(ResumeEditIntent.HideAntiPatternSheet) },
            ...
            Column(
                ...
                antiPatterns.forEach { item ->
                    SelectionItemCard(
                        ...
                        onClick = {
                            onIntent(
                                ResumeEditIntent.SelectAntiPattern(
                                    item.title,
                                    mockAntiPatternFeedback[item.title]
    ...
    if (state.showPersonaSheet) {
        ModalBottomSheet(
            onDismissRequest = { onIntent(ResumeEditIntent.HidePersonaSheet) },
            sheetState = rememberModalBottomSheetState(skipPartiallyExpanded = true),
            containerColor = Color.White,
            dragHandle = { BottomSheetDefaults.DragHandle() }
        ) {
            Column(
                ...
                when {
                    ...
                    state.personasError != null -> {
                        Text(
                            text = state.personasError ?: "",
                            color = colors.ErrorRed,
                            fontSize = 14.sp,
                            modifier = Modifier.padding(vertical = 8.dp)
                        )
                        TextButton(onClick = onLoadPersonas) {
                            Text("다시 시도", color = colors.PrimaryBlue)
                        }
                    }
                    else -> {
                        state.personas.forEach { persona ->
                            SelectionItemCard(
                                ...
                                onClick = {
                                    onIntent(
                                        ResumeEditIntent.SelectPersona(
                                            persona.title,
                                            mockPersonaFeedback[persona.title]
...
private fun ResumeEditScreenPreview(
    @PreviewParameter(ResumeEditStateProvider::class) state: ResumeEditState
) {
    MaterialTheme {
        ResumeEditContent(
            state = state,
